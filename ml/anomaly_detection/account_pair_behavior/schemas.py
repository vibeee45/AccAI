...
class AccountPairBehaviorFeatures:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.account_pair.strip():
            raise ValueError("account_pair cannot be empty.")
        ...
        if not 0.0 <= self.pair_ratio <= 1.0:
            raise ValueError("pair_ratio must be between 0 and 1.")
...
class AccountPairHistoryItem:
    ...
    def __post_init__(self) -> None:
        ...
        if not self.debit_account_id.strip():
            raise ValueError("debit_account_id cannot be empty.")

        if not self.credit_account_id.strip():
            raise ValueError("credit_account_id cannot be empty.")
